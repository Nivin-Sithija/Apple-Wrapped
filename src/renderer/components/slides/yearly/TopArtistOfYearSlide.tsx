...
export function TopArtistOfYearSlide({ yearRecap, palette }: SlideProps) {
  if (!yearRecap) return null
  const top = yearRecap.topArtists[0]

  if (!top) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-7">
        <p className="opacity-50 text-sm">No plays recorded this year.</p>
      </div>
    )
  }
